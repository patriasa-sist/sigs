...
export const CHECKLIST_DEFINICION: Omit<ChecklistItem, "estado" | "vigente" | "subsidios">[] = [
	...
	{ id: "item_15", numero: 15, label: "Carta de Confidencialidad de Información", seccion: "recepcionada" },
	{
		id: "item_16",
		numero: 16,
		label: "Acuse de Recibo de Manual Interno y Código de Ética",
		seccion: "recepcionada",
	},
	...
	{ id: "item_18", numero: 18, label: "Contrato de Trabajo", seccion: "adjunto" },
	{
		id: "item_19",
		numero: 19,
		label: "Descripción y Aceptación de Funciones del Cargo conforme Manual Interno",
		seccion: "adjunto",
	},
	{
		id: "item_20",
		numero: 20,
		label: "Recepción y Aceptación del Manual Interno de Prevención LGI/FT/FPADM",
		seccion: "adjunto",
	},
	{
		id: "item_21",
		numero: 21,
		label: "Recepción y Aceptación del Código de Ética y Conducta de la Empresa",
		seccion: "adjunto",
	},
	...
	{ id: "item_29", numero: 29, label: "Pago de Indemnizaciones (Anticipo Solicitado)", seccion: "otros" },
	{
		id: "item_30",
		numero: 30,
		label: "Certificados de Incapacidad Temporal y/o Certificación Médica",
		seccion: "otros",
	},
...
export function crearChecklistVacio(): Record<string, ChecklistItem> {
	...
	for (const def of CHECKLIST_DEFINICION) {
		...
		if (def.id === "item_33")
			item.subsidios = { prenatal: false, natalidad: false, lactancia: false, sepelio: false };
		result[def.id] = item;
	}
	return result;
}
...
export function calcularCompletitudChecklist(items: Record<string, ChecklistItem>): number {
	...
	const completados = CHECKLIST_DEFINICION.filter((def) => items[def.id]?.estado !== null).length;
