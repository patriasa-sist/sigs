...
export const CLUB_REGISTRY_TYPES = ["municipal", "gobernacion", "viceministerio_de_deportes", "otra"] as const;
export type ClubRegistryType = (typeof CLUB_REGISTRY_TYPES)[number];

// Tipos de asociación civil (sin fines de lucro)
export const ASOCIACION_CIVIL_TYPES = ["sociedad_profesional", "asociacion_gremial", "fundacion", "otra"] as const;
...
export const naturalClientOtherSchema = z.object({
	...
	nivel_ingresos: z.preprocess((val) => {
...
export interface NaturalClientFormData
	extends NaturalClientPersonalData, NaturalClientContactData, NaturalClientOtherData {
...
export interface JuridicClientFormData extends JuridicClientCompanyData, JuridicClientContactData {
	legal_representatives: LegalRepresentativeData[];
	documentos?: ClienteDocumentoFormState[];
	celulares_extra?: ExtraPhone[];
}

export const juridicClientFormSchema = juridicClientCompanySchema.merge(juridicClientContactSchema).extend({
...
export const ongClientFormSchema = ongClientOrgSchema.merge(ongClientContactSchema).merge(ongRepresentativeSchema);
...
export interface ClubClientFormData extends ClubClientOrgData, ClubClientContactData, ClubRepresentativeData {
	documentos?: ClienteDocumentoFormState[];
	celulares_extra?: ExtraPhone[];
}

export const clubClientFormSchema = clubClientOrgSchema.merge(clubClientContactSchema).merge(clubRepresentativeSchema);
...
export interface AsociacionCivilClientFormData
	extends AsociacionCivilOrgData, AsociacionCivilContactData, AsociacionCivilRepresentativeData {
	documentos?: ClienteDocumentoFormState[];
	celulares_extra?: ExtraPhone[];
}
