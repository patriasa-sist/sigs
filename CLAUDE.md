...
- Consulta global de pólizas: al **buscar** (texto en el listado) el alcance se abre a todas las pólizas y su detalle completo —incluidos anexos y asegurados— queda visible en SOLO LECTURA, marcado con badge "Otro equipo"/"Solo consulta"; sin búsqueda, el listado y los filtros siguen scopeados. Lectura fuera de alcance vía `clienteLecturaGlobal()` (`utils/polizas/lecturaGlobal.ts`, service role solo para SELECT: `polizas`, `polizas_pagos` y `polizas_documentos` son las únicas tablas con RLS scopeada). Las acciones (editar, anexos, validar, pagos, permisos) siguen bloqueadas en UI y servidor.
