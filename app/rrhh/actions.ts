...
import type { EmpleadoFormState, ChecklistItem, EmployeeDetail, EmployeeListItem } from "@/types/rrhh";
...
export async function obtenerEmpleado(id: string): Promise<{
	...
	const patrimony = Array.isArray(patrimonyArr) && patrimonyArr.length > 0 ? patrimonyArr[0] : null;

	const checklistArr = raw.employee_checklist as Record<string, unknown>[] | null;
	const checklist = Array.isArray(checklistArr) && checklistArr.length > 0 ? checklistArr[0] : null;
		...
		if (!patError && pat) {
			...
			if (items.length > 0) {
				const rows = items.map((i) => ({
					...
					fecha_vencimiento:
						"fecha_vencimiento" in i
							? (i as { fecha_vencimiento: string }).fecha_vencimiento || null
							: null,
					valor: i.valor ?? 0,
					orden: i.orden,
				}));
...
export async function actualizarChecklist(
	...
	const { error } = await supabase
		...
		.upsert({ employee_id: employeeId, items, updated_by: user.id }, { onConflict: "employee_id" });
...
export async function subirDocumentoEmpleado(
	...
	const { error: uploadError } = await supabase.storage.from("rrhh-documentos").upload(path, file);
...
export async function descartarDocumentoEmpleado(
	...
	const { error } = await supabase.from("employee_documents").update({ estado: "descartado" }).eq("id", documentId);
...
// -----------------------------------------------
export async function obtenerUrlDocumentoEmpleado(path: string): Promise<{ url: string | null; error: string | null }> {
	const supabase = await createClient();
	const { data, error } = await supabase.storage.from("rrhh-documentos").createSignedUrl(path, 3600); // 1 hora
...
// -----------------------------------------------
export async function subirCroquis(formData: FormData): Promise<{ success: boolean; path?: string; error?: string }> {
	const supabase = await createClient();
	const {
		data: { user },
	} = await supabase.auth.getUser();
	...
	const { error } = await supabase.storage.from("rrhh-documentos").upload(path, file);
