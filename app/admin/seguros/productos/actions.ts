...
const productoSchema = z.object({
	...
	factor_contado: z.number().positive("El factor debe ser positivo").max(100, "El factor no puede exceder 100"),
	factor_credito: z.number().positive("El factor debe ser positivo").max(100, "El factor no puede exceder 100"),
...
export async function obtenerProductosStats(): Promise<CatalogoStats> {
	...
	const [{ count: total }, { count: activos }] = await Promise.all([
		supabase.from("productos_aseguradoras").select("*", { count: "exact", head: true }),
		supabase.from("productos_aseguradoras").select("*", { count: "exact", head: true }).eq("activo", true),
	]);
...
export async function reactivarProducto(id: string): Promise<CatalogoActionResult> {
	const supabase = await createClient();
	...
	const { error } = await supabase.from("productos_aseguradoras").update({ activo: true }).eq("id", id);
