...
export async function obtenerRamos(includeInactive: boolean = false): Promise<TipoSeguroConHijos[]> {
...
export async function obtenerRamosStats(): Promise<CatalogoStats & { padres: number; hijos: number }> {
	const supabase = await createClient();

	const [{ count: total }, { count: activos }, { count: padres }] = await Promise.all([
		supabase.from("tipos_seguros").select("*", { count: "exact", head: true }),
		supabase.from("tipos_seguros").select("*", { count: "exact", head: true }).eq("activo", true),
		supabase
			.from("tipos_seguros")
...
export async function crearRamo(data: TipoSeguroForm): Promise<CatalogoActionResult<TipoSeguroDB>> {
	const supabase = await createClient();
...
export async function actualizarRamo(id: number, data: TipoSeguroForm): Promise<CatalogoActionResult<TipoSeguroDB>> {
	const supabase = await createClient();
	...
	const { data: currentRamo } = await supabase.from("tipos_seguros").select("es_ramo_padre").eq("id", id).single();
...
export async function desactivarRamo(id: number): Promise<CatalogoActionResult> {
	...
	const { error } = await supabase.from("tipos_seguros").update({ activo: false }).eq("id", id);
...
export async function reactivarRamo(id: number): Promise<CatalogoActionResult> {
	...
	// Get ramo info
	const { data: ramo } = await supabase.from("tipos_seguros").select("ramo_padre_id").eq("id", id).single();
	...
	const { error } = await supabase.from("tipos_seguros").update({ activo: true }).eq("id", id);
