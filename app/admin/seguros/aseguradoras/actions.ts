...
export async function obtenerAseguradoras(includeInactive: boolean = false): Promise<CompaniaAseguradoraDB[]> {
	const supabase = await createClient();

	let query = supabase.from("companias_aseguradoras").select("*").order("nombre", { ascending: true });
...
export async function obtenerAseguradorasStats(): Promise<CatalogoStats> {
	...
	const [{ count: total }, { count: activos }] = await Promise.all([
		...
		supabase.from("companias_aseguradoras").select("*", { count: "exact", head: true }).eq("activo", true),
	]);
...
export async function desactivarAseguradora(id: string): Promise<CatalogoActionResult> {
	const supabase = await createClient();
	...
	const { error } = await supabase.from("companias_aseguradoras").update({ activo: false }).eq("id", id);
...
export async function reactivarAseguradora(id: string): Promise<CatalogoActionResult> {
	const supabase = await createClient();

	const { error } = await supabase.from("companias_aseguradoras").update({ activo: true }).eq("id", id);
