...
import { z } from "zod";
import type { CategoriaDB, CategoriaForm, CatalogoActionResult, CatalogoStats } from "@/types/catalogo-seguros";
...
export async function obtenerCategorias(includeInactive: boolean = false): Promise<CategoriaDB[]> {
...
export async function obtenerCategoriasStats(): Promise<CatalogoStats> {
	...
	const [{ count: total }, { count: activos }] = await Promise.all([
		...
		supabase.from("categorias").select("*", { count: "exact", head: true }).eq("activo", true),
	]);
...
export async function crearCategoria(data: CategoriaForm): Promise<CatalogoActionResult<CategoriaDB>> {
	const supabase = await createClient();
...
export async function actualizarCategoria(id: string, data: CategoriaForm): Promise<CatalogoActionResult<CategoriaDB>> {
	const supabase = await createClient();
...
export async function desactivarCategoria(id: string): Promise<CatalogoActionResult> {
	const supabase = await createClient();
...
export async function reactivarCategoria(id: string): Promise<CatalogoActionResult> {
	const supabase = await createClient();

	const { error } = await supabase.from("categorias").update({ activo: true }).eq("id", id);
