...
export async function generateLetterReference(): Promise<string> {
  try {
    ...
    let executive = getDefaultExecutive(); // Default to Tamara
    ...
    if (existingRef) {
      ...
      if (sequentialNumber > 99999) {
        throw new Error(`Maximum letter count (99999) reached for ${executive.glyph} in ${year}-${month.toString().padStart(2, '0')}`);
      }
