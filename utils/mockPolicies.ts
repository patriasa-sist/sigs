...
function generatePayments(paymentCount: number, totalPremium: number, startDate: Date): Payment[] {
...
export function getPaymentStats(policy: Policy) {
	...
	const totalPaid = policy.payments.filter((p) => p.isPaid).reduce((sum, p) => sum + p.amount, 0);
