...
function AnexoAutomotor({
	...
	if (tipoAnexo === "exclusion") {
		...
		return (
			<div>
				<p className="text-sm text-gray-600 mb-4">Seleccione los vehículos que desea excluir de la póliza:</p>
				<ExclusionSelector
					items={itemsActuales}
	...
	return (
		<div>
			<p className="text-sm text-gray-600 mb-4">Agregue los nuevos vehículos que desea incluir en la póliza:</p>
							...
							<div className="flex gap-1">
								...
								<button
									onClick={() => {
										...
										onChange(
											newItems.map((it) => ({ accion: "inclusion" as const, data: it.data })),
										);
									}}
...
function AnexoRamosTecnicos({
	...
	if (tipoAnexo === "exclusion") {
		...
		return (
			<div>
				<p className="text-sm text-gray-600 mb-4">Seleccione los equipos que desea excluir de la póliza:</p>
				<ExclusionSelector
					items={itemsActuales}
					selectedIds={selectedIds}
					onToggle={handleToggle}
	...
	return (
		<div>
			<p className="text-sm text-gray-600 mb-4">Agregue los nuevos equipos que desea incluir en la póliza:</p>
...
function AnexoAeronavegacion({
	...
	if (tipoAnexo === "exclusion") {
		...
		return (
			<div>
				<p className="text-sm text-gray-600 mb-4">Seleccione las {labelPlural} que desea excluir:</p>
				<ExclusionSelector
					items={itemsActuales}
					selectedIds={selectedIds}
					onToggle={handleToggle}
	...
	return (
		<div>
			<p className="text-sm text-gray-600 mb-4">Agregue las nuevas {labelPlural} que desea incluir:</p>
			...
			{inclusiones.length > 0 && (
				<div className="mb-4 space-y-2">
					<p className="text-xs text-gray-500 mb-1 font-medium">
						{tipoNave === "aeronave" ? "Aeronaves" : "Embarcaciones"} a incluir:
					</p>
					{inclusiones.map((item, idx) => (
						<div
							key={idx}
...
function AnexoSalud({
	...
	const [selectedAseguradoIds, setSelectedAseguradoIds] = useState<Set<string>>(
		new Set(
			itemsCambio?.items_asegurados.filter((i) => i.accion === "exclusion").map((i) => i.original_item_id!) || [],
		),
	);
	const [selectedBeneficiarioIds, setSelectedBeneficiarioIds] = useState<Set<string>>(
		new Set(
			itemsCambio?.items_beneficiarios.filter((i) => i.accion === "exclusion").map((i) => i.original_item_id!) ||
				[],
		),
	);
			...
			<div>
				...
				{asegurados.length === 0 && beneficiarios.length === 0 && (
					<p className="text-gray-400 text-sm text-center py-4">
						No hay asegurados ni beneficiarios registrados
					</p>
				)}
			</div>
		);
	...
	return (
		<div>
			...
			<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-800">
				...
				La inclusión de asegurados en pólizas de {ramo} requiere la configuración de niveles de cobertura. Esta
				funcionalidad estará disponible próximamente. Puede continuar y registrar el ajuste de pagos y
				documentos.
			</div>
		</div>
	);
...
function AnexoBienes<T extends { direccion: string; valor_total_declarado: number }>({
	...
	if (tipoAnexo === "exclusion") {
		...
		return (
			<div>
				<p className="text-sm text-gray-600 mb-4">Seleccione los bienes que desea excluir de la póliza:</p>
				<ExclusionSelector
					items={itemsActuales}
					selectedIds={selectedIds}
					onToggle={handleToggle}
	...
	return (
		<div>
			...
			<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-800">
				...
				Esta funcionalidad estará disponible próximamente. Puede continuar y registrar el ajuste de pagos y
				documentos.
			</div>
		</div>
	);
...
function AnexoAseguradosNivel({
	...
	if (tipoAnexo === "exclusion") {
		...
		return (
			<div>
				<p className="text-sm text-gray-600 mb-4">Seleccione los asegurados que desea excluir de la póliza:</p>
				<ExclusionSelector
					items={itemsActuales}
					selectedIds={selectedIds}
					onToggle={handleToggle}
