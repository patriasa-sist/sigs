...
export function VidaForm({ datos, regionales, aseguradoPrincipal, onChange, onSiguiente, onAnterior }: Props) {
	...
	const guardarNivel = () => {
		...
		const alguna = Object.values(coberturas).some((c) => c.habilitado);
		if (!alguna) {
			nuevosErrores.general = "Debe habilitar al menos una cobertura";
		}

		const conValorCero = Object.entries(coberturas).filter(([, c]) => c.habilitado && c.valor <= 0);
		conValorCero.forEach(([key]) => {
			nuevosErrores[key] = "El valor de la cobertura debe ser mayor a 0";
		});
	...
	const cambiarRolContratante = (rol: ContratanteAPVida["rol"]) => {
		if (!contratante) return;
		setContratante({ ...contratante, rol });
	};

	const cambiarNivelContratante = (nivelId: string) => {
		if (!contratante) return;
		setContratante({ ...contratante, nivel_id: nivelId });
	};

	const eliminarContratante = () => {
		setContratante(null);
	};

	// ===== FUNCIONES PASO 3: ASEGURADOS (datos mínimos) =====
	const abrirModalAsegurado = () => {
		setAseguradoEditando(null);
		setMostrarModalAsegurado(true);
	};

	const editarAsegurado = (asegurado: AseguradoAPVida) => {
		setAseguradoEditando(asegurado);
		setMostrarModalAsegurado(true);
	};
	...
	const handleContinuar = () => {
		...
		if (!contratante) {
			...
			if (!contratante.nivel_id) {
				nuevosErrores.contratante = "El contratante debe tener un nivel asignado";
			}
				...
				<AlertDialogHeader>
					<AlertDialogTitle>
						{confirmacionEliminar?.tipo === "nivel"
							...
							: "¿Está seguro de eliminar este asegurado?"}
					</AlertDialogTitle>
									...
									{coberturas.muerte.habilitado && (
										<div className="flex-1">
											<Input
												...
												className={errores.muerte ? "border-destructive" : ""}
											/>
											{errores.muerte && (
												<p className="text-sm text-destructive mt-1">{errores.muerte}</p>
											)}
										</div>
									)}
								</div>
									...
									{coberturas.dima.habilitado && (
										<div className="flex-1">
											<Input
												...
												className={errores.dima ? "border-destructive" : ""}
											/>
											{errores.dima && (
												<p className="text-sm text-destructive mt-1">{errores.dima}</p>
											)}
										</div>
									)}
								</div>
									...
									{coberturas.indm_enfermedades_graves.habilitado && (
										<div className="flex-1">
											<Input
												...
												className={errores.indm_enfermedades_graves ? "border-destructive" : ""}
											/>
	...
	return (
		<div className="bg-card rounded-lg shadow-sm border border-border p-6">
			<div className="flex items-center justify-between mb-6">
				<div>
					<h2 className="text-xl font-semibold text-foreground">Paso 3: Datos Específicos — Vida</h2>
					<p className="text-sm text-muted-foreground mt-1">
			...
			<div className="space-y-4 mb-6">
				...
				{/* Sugerencia: asegurado principal como contratante */}
				...
				{/* Buscador de cliente para contratante */}
							...
							<div className="flex-1 space-y-3">
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
									<div className="space-y-1">
										...
										<Select value={contratante.nivel_id} onValueChange={cambiarNivelContratante}>
			...
			<div className="bg-info/10 border border-info/20 rounded-lg p-4 mb-6">
				<p className="text-sm text-info font-medium mb-2">Roles en pólizas de Vida:</p>
