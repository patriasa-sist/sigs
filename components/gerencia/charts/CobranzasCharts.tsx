...
const COLORS = ["#16a34a", "#f59e0b", "#dc2626", "#6366f1"];
							...
							<ResponsiveContainer width="100%" height={300}>
								<PieChart>
									<Pie
										...
										dataKey="cantidad"
										nameKey="estado"
										label={({ name, value }) => `${ESTADO_LABELS[name as string] || name}: ${value}`}
									>
									...
									<Tooltip
										formatter={(value, name) => [
											value,
											ESTADO_LABELS[name as string] || name,
										]}
									/>
							...
							<TableBody>
								{proximasCuotas.map((c, i) => (
									<TableRow key={i}>
										...
										<TableCell>{new Date(c.fecha_vencimiento).toLocaleDateString("es-BO")}</TableCell>
									</TableRow>
								))}
							</TableBody>
						</Table>
