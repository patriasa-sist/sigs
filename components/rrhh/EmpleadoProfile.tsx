...
import {
	...
	calcularCompletitudChecklist,
	calcularTotalesPatrimonio,
} from "@/types/rrhh";
import {
	actualizarChecklist,
	subirDocumentoEmpleado,
	descartarDocumentoEmpleado,
	obtenerUrlDocumentoEmpleado,
} from "@/app/rrhh/actions";
...
export default function EmpleadoProfile({ empleado }: Props) {
	...
	const totales = patrimony
		? calcularTotalesPatrimonio({
				...
				inmuebles: (patrimony.employee_patrimony_items ?? [])
					.filter((i) => i.categoria === "inmueble")
					.map((i) => ({
						id: i.id,
				...
				vehiculos: (patrimony.employee_patrimony_items ?? [])
					.filter((i) => i.categoria === "vehiculo")
					.map((i) => ({
						id: i.id,
						descripcion: "",
						ubicacion: "",
						modelo_marca: i.modelo_marca ?? "",
						placa: i.placa ?? "",
						entidad: "",
						tipo_deuda: "",
						fecha_vencimiento: "",
				...
				otros_bienes: (patrimony.employee_patrimony_items ?? [])
					.filter((i) => i.categoria === "otro_bien")
					.map((i) => ({
						id: i.id,
						descripcion: i.descripcion ?? "",
						ubicacion: "",
				...
				deudas: (patrimony.employee_patrimony_items ?? [])
					.filter((i) => i.categoria === "deuda")
					.map((i) => ({
						id: i.id,
						descripcion: i.descripcion ?? "",
						ubicacion: "",
			...
			{showPDF === "identificacion" && (
				<div className="border rounded-lg overflow-hidden">
					<div className="flex justify-end p-2 bg-muted">
						<Button variant="ghost" size="sm" onClick={() => setShowPDF(null)}>
							Cerrar
						</Button>
					</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div className="rounded-lg border p-4">
						...
						<dl className="space-y-0.5">
							...
							<Row
								label="C.I."
								value={`${empleado.nro_documento}${empleado.complemento ? ` ${empleado.complemento}` : ""}${empleado.extension ? ` — ${empleado.extension}` : ""}`}
							/>
							...
							<Row
								...
								value={
									empleado.fecha_nacimiento
										? new Date(empleado.fecha_nacimiento).toLocaleDateString("es-BO")
										: null
								}
							/>
							...
							<Row
								label="Estado civil"
								value={empleado.estado_civil ? LABEL_ESTADO_CIVIL[empleado.estado_civil] : null}
							/>
					...
					<div className="rounded-lg border p-4">
						...
						<dl className="space-y-0.5">
							...
							{empleado.lat && (
								<Row
									...
									value={`${Number(empleado.lat).toFixed(6)}, ${Number(empleado.lng).toFixed(6)}`}
								/>
							)}
						</dl>
						...
						{empleado.croquis_url && (
							<div className="mt-3">
								...
								<img
									src={empleado.croquis_url}
									alt="Croquis domicilio"
					...
					<div className="rounded-lg border p-4">
						...
						<dl className="space-y-0.5">
							...
							<Row
								label="Fecha ingreso"
								value={new Date(empleado.fecha_ingreso).toLocaleDateString("es-BO")}
							/>
							<Row
								label="Haber básico"
								value={
							...
							<Row
								...
								value={
									empleado.area_solicitante
										? LABEL_AREA[empleado.area_solicitante as keyof typeof LABEL_AREA]
										: null
								}
							/>
							<Row
								...
								value={
									empleado.medio_comunicacion
										? LABEL_MEDIO[empleado.medio_comunicacion as keyof typeof LABEL_MEDIO]
										: null
								}
							/>
						...
						<>
									...
									[
										...
										<div key={t.label} className="rounded-lg border p-3">
											...
											<p className="text-lg font-semibold mt-1">
												Bs.{" "}
												{Number(t.v ?? 0).toLocaleString("es-BO", { minimumFractionDigits: 2 })}
											</p>
										</div>
									))}
							</div>
							...
							{[
								{
									cat: "inmueble",
									title: "Inmuebles",
									cols: ["Descripción", "Ubicación", "Valor (Bs.)"],
								},
								...
								{ cat: "otro_bien", title: "Otros Bienes", cols: ["Descripción", "Valor (Bs.)"] },
								{
									...
									cols: ["Entidad", "Tipo", "Vencimiento", "Saldo (Bs.)"],
								},
							].map(({ cat, title }) => {
								const items = (patrimony.employee_patrimony_items ?? []).filter(
									(i) => i.categoria === cat,
								);
								if (items.length === 0) return null;
								return (
									<div key={cat} className="rounded-lg border overflow-x-auto">
										<div className="px-4 py-2 bg-muted/40 border-b text-sm font-medium">
											{title}
										</div>
							...
							{patrimony.lugar_fecha && (
								<p className="text-xs text-muted-foreground">
									...
									{patrimony.fecha_declaracion &&
										` — ${new Date(patrimony.fecha_declaracion).toLocaleDateString("es-BO")}`}
								</p>
							)}
						</>
					)}
										...
										return (
											...
											>
												...
												<div className="flex gap-1 shrink-0">
													<button
														onClick={() =>
															updateItem(def.id, item?.estado === "si" ? null : "si")
														}
													...
													<button
														onClick={() =>
															updateItem(def.id, item?.estado === "no" ? null : "no")
														}
														className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium transition-colors ${
															item?.estado === "no"
					...
					{activeDocs.length === 0 ? (
						...
						<div className="space-y-2">
							{activeDocs.map((doc) => (
								<div
									key={doc.id}
									className="flex items-center gap-3 px-4 py-3 border rounded-lg bg-card"
								>
