...
export default function StepPatrimonio({ data, onSave, onBack }: Props) {
	...
	const set = <K extends keyof DatosPatrimonio>(k: K, v: DatosPatrimonio[K]) => setForm((p) => ({ ...p, [k]: v }));
			...
			<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
				...
				<div>
					...
					<Input
						...
						className="mt-1"
						placeholder="Ej: Santa Cruz, 18 de febrero de 2025"
					/>
				</div>
				<div>
					...
					<Input
						...
						onChange={(e) => set("fecha_declaracion", e.target.value)}
						className="mt-1"
					/>
				</div>
			</div>
				...
				{[
					...
					<div key={t.label}>
						...
						<p className="font-semibold">
							Bs. {t.value.toLocaleString("es-BO", { minimumFractionDigits: 2 })}
						</p>
					</div>
				))}
			</div>
