	...
	return (
		<div className="space-y-6">
			...
			{identificacion && (
				<div className="rounded-lg border p-4">
					...
					<dl className="space-y-1.5">
						...
						<Row
							label="Documento"
							value={`${identificacion.nro_documento}${identificacion.extension ? ` — ${identificacion.extension}` : ""}`}
						/>
						...
						<Row
							...
							value={
								identificacion.fecha_nacimiento
									? new Date(identificacion.fecha_nacimiento).toLocaleDateString("es-BO")
									: null
							}
						/>
						...
						<Row
							label="Nacionalidad"
							value={identificacion.otra_nacionalidad || identificacion.nacionalidad}
						/>
						<Row
							label="Estado civil"
							value={identificacion.estado_civil ? LABEL_ESTADO_CIVIL[identificacion.estado_civil] : null}
						/>
			...
			{direccion && (
				<div className="rounded-lg border p-4">
					...
					<dl className="space-y-1.5">
						<Row
							label="Dirección"
							value={[direccion.av_calle_pasaje, direccion.zona_barrio, direccion.departamento]
								.filter(Boolean)
								.join(", ")}
						/>
						...
						{direccion.lat && (
							<Row
								label="Ubicación GPS"
								value={`${direccion.lat.toFixed(5)}, ${direccion.lng?.toFixed(5)}`}
							/>
						)}
						{direccion.referencias[0]?.nombres_apellidos && (
							<Row
								label="Ref. Familiar 1"
								value={`${direccion.referencias[0].nombres_apellidos} — ${direccion.referencias[0].parentesco || "s/d"}`}
							/>
						)}
						{direccion.referencias[1]?.nombres_apellidos && (
							<Row
								label="Ref. Familiar 2"
								value={`${direccion.referencias[1].nombres_apellidos} — ${direccion.referencias[1].parentesco || "s/d"}`}
							/>
						)}
					</dl>
				</div>
						...
						<Row
							...
							value={
								laboral.fecha_ingreso
									? new Date(laboral.fecha_ingreso).toLocaleDateString("es-BO")
									: null
							}
						/>
						<Row
							label="Haber básico"
							value={laboral.haber_basico ? `Bs. ${laboral.haber_basico.toLocaleString("es-BO")}` : null}
						/>
						<Row
							label="Área solicitante"
							value={laboral.area_solicitante ? LABEL_AREA[laboral.area_solicitante] : null}
						/>
			...
			{patrimonio && totales && (
				<div className="rounded-lg border p-4">
					...
					<dl className="space-y-1.5">
						...
						<Row
							label="Activo total"
							value={`Bs. ${totales.activo_total.toLocaleString("es-BO", { minimumFractionDigits: 2 })}`}
						/>
						<Row
							label="Patrimonio neto"
							value={`Bs. ${totales.patrimonio_neto.toLocaleString("es-BO", { minimumFractionDigits: 2 })}`}
						/>
					</dl>
				</div>
			)}
						...
						{documentos.map((d) => (
							<li key={d.id} className="text-xs flex items-center gap-2">
								...
								<span className="text-muted-foreground">
									— {TIPOS_DOCUMENTO_EMPLEADO[d.tipo_documento]}
								</span>
							</li>
						))}
					</ul>
			...
			<div className="flex justify-between pt-2">
				...
				<Button onClick={onSave} disabled={saving || hasBlockers} className="gap-2">
