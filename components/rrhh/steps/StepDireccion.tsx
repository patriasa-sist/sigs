...
export default function StepDireccion({ data, onSave, onBack }: Props) {
	...
	const set = <K extends keyof DatosDireccion>(k: K, v: DatosDireccion[K]) => setForm((p) => ({ ...p, [k]: v }));

	const setRef = (idx: 0 | 1, k: keyof ReferenciaFamiliar, v: string) =>
		setForm((p) => {
			const refs: [ReferenciaFamiliar, ReferenciaFamiliar] = [...p.referencias] as [
				ReferenciaFamiliar,
				ReferenciaFamiliar,
			];
			...
			<div>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div className="sm:col-span-2">
						...
						<Input
							value={form.av_calle_pasaje}
							onChange={(e) => set("av_calle_pasaje", e.target.value)}
							className="mt-1"
						/>
					</div>
					<div>
						...
						<Input
							...
							onChange={(e) => set("zona_barrio", e.target.value)}
							className="mt-1"
						/>
					</div>
					<div>
						...
						<Input
							value={form.urbanizacion_condominio}
							onChange={(e) => set("urbanizacion_condominio", e.target.value)}
							className="mt-1"
						/>
					</div>
					<div>
						...
						<Input
							value={form.edif_bloque_piso}
							onChange={(e) => set("edif_bloque_piso", e.target.value)}
							className="mt-1"
						/>
					</div>
					<div>
					...
					<div className="sm:col-span-2">
						...
						<Input
							...
							className="mt-1"
							placeholder="Ej: A 2 cuadras de la plaza"
						/>
					</div>
					<div>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div>
						...
						>
							...
							{form.lat && form.lng ? `${form.lat.toFixed(5)}, ${form.lng.toFixed(5)}` : "Abrir mapa"}
						</Button>
						{form.lat && form.lng && (
						...
						{form.croquis_preview ? (
							<div className="relative mt-1">
								...
								<button
									type="button"
									onClick={() => {
										set("croquis_file", null);
										set("croquis_preview", null);
									}}
