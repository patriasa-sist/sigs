			...
			<Page size="LETTER" style={S.page}>
				...
				<View style={{ marginBottom: 10 }}>
					{[
						...
						{
							n: "2",
							l: "Cédula de Identidad",
							v: `${empleado.nro_documento}${empleado.extension ? ` — ${empleado.extension}` : ""}`,
						},
						{ n: "3", l: "Número de NUA/CUA", v: empleado.nro_nua_cua ?? "" },
						{
							n: "4",
							l: "Fecha de Ingreso",
							v: empleado.fecha_ingreso
									...
									<View key={def.id} style={S.itemRow}>
										...
										{def.id === "item_33" && (
											<View style={{ flexDirection: "row", gap: 6 }}>
												{(["prenatal", "natalidad", "lactancia", "sepelio"] as const).map(
													(sub) => (
														<View
															key={sub}
				...
				<View style={{ marginTop: 16, alignItems: "flex-end" }}>
					<View style={{ borderTop: "0.5 solid #333", width: 160, paddingTop: 4 }}>
						<Text style={{ fontSize: 7, color: "#555", textAlign: "center" }}>
							Recursos Humanos (Sello y Firma)
						</Text>
					</View>
					<View style={{ flexDirection: "row", gap: 6, marginTop: 4, alignItems: "center" }}>
						...
						{["Día", "Mes", "Año"].map((l) => (
							<View
								key={l}
								style={{ border: "0.5 solid #333", padding: "2 6", minWidth: 24, alignItems: "center" }}
							>
