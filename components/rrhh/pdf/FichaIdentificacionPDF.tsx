	...
	return (
		<View style={[S.checkRow, { marginLeft: 8 }]}>
			<View style={S.checkBox}>
				{checked && <Text style={{ fontSize: 8, color: "#003366", textAlign: "center" }}>✓</Text>}
			</View>
		...
		<Document title={`Ficha Identificación — ${empleado.apellidos} ${empleado.nombres}`}>
			<Page size="LETTER" style={S.page}>
				...
				<View style={S.section}>
					...
					<View style={S.rowFull}>
						<View style={S.col2}>
							...
							<Field
								label="5  Nro. Documento"
								value={`${empleado.nro_documento}${empleado.complemento ? ` ${empleado.complemento}` : ""}`}
							/>
							<Field label="6  Nro. NUA/CUA" value={empleado.nro_nua_cua ?? ""} />
							<Field label="7  NIT" value={empleado.nit ?? ""} />
							<Field
								label="8  Fecha Nacimiento"
								value={
						...
						<View style={S.col2}>
							...
							<View style={S.row}>
								...
								<View style={{ flex: 1, flexDirection: "row", flexWrap: "wrap", gap: 4 }}>
									{(["soltero", "casado", "union_libre", "divorciado", "viudo"] as const).map(
										(ec) => (
											<CheckField
												key={ec}
												label={LABEL_ESTADO_CIVIL[ec]}
												checked={empleado.estado_civil === ec}
											/>
										),
									)}
								</View>
				...
				<View style={S.section}>
					...
					<View style={S.rowFull}>
						<View style={S.col2}>
							<Field label="18 Departamento" value={empleado.departamento ?? ""} />
						</View>
						<View style={S.col2}>
							<Field label="19 País" value={empleado.pais ?? "Bolivia"} />
						</View>
					</View>
					{empleado.lat && (
						<Field
						...
						{[0, 1].map((idx) => {
							...
							return (
								<View key={idx} style={[S.col2, { paddingLeft: idx === 1 ? 8 : 0 }]}>
									<Text style={[S.checkLabel, { fontFamily: "Helvetica-Bold", marginBottom: 2 }]}>
										{idx + 1}
									</Text>
				...
				<View style={S.section}>
					...
					<View style={S.row}>
						...
						<View style={{ flex: 1, flexDirection: "row", gap: 8 }}>
							...
							<CheckField
								label="Invitación Personal"
								checked={empleado.medio_comunicacion === "invitacion_personal"}
							/>
					...
					<View style={S.row}>
						...
						<View style={{ flex: 1, flexDirection: "row", gap: 6, flexWrap: "wrap" }}>
							{(
								["comercial_produccion", "cobranzas", "reclamos", "recepcion", "contabilidad"] as const
							).map((a) => (
					...
					<View style={S.rowFull}>
						<View style={S.col2}>
							<Field
								label="28 Entrevistado por — Nombre"
								value={empleado.entrevistado_por_nombre ?? ""}
							/>
							<Field label="   Cargo" value={empleado.entrevistado_por_cargo ?? ""} />
							<Field
								label="   Fecha"
								value={
									empleado.entrevistado_fecha
										? new Date(empleado.entrevistado_fecha).toLocaleDateString("es-BO")
										: ""
								}
							/>
						</View>
						<View style={S.col2}>
							...
							<Field
								label="   Fecha"
								value={
									empleado.aprobado_fecha
										? new Date(empleado.aprobado_fecha).toLocaleDateString("es-BO")
										: ""
								}
							/>
						</View>
				...
				<View style={S.section}>
					...
					<View style={S.rowFull}>
						<View style={S.col2}>
							<Field
								label="30 Fecha de Ingreso"
								value={
									empleado.fecha_ingreso
										? new Date(empleado.fecha_ingreso).toLocaleDateString("es-BO")
										: ""
								}
							/>
						</View>
						<View style={S.col2}>
							<Field
								label="31 Haber Básico"
								value={
									empleado.haber_basico
										? `Bs. ${Number(empleado.haber_basico).toLocaleString("es-BO", { minimumFractionDigits: 2 })}`
										: ""
								}
							/>
						</View>
				...
				<View style={S.footer}>
					<View style={S.signLine}>
						<Text>Firma — Nombre — CI</Text>
					</View>
					<View style={S.signLine}>
						<Text>Recursos Humanos (Sello y Firma)</Text>
					</View>
					<View style={S.signLine}>
						<Text>Aprobado por Gerencia (Sello y Firma)</Text>
					</View>
				</View>
