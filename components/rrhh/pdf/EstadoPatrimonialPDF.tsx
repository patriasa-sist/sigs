			...
			<Page size="LETTER" style={S.page}>
				...
				<View style={S.headerRow}>
					...
					<View style={{ flex: 1, paddingLeft: 16 }}>
						...
						<Text style={S.subtitle}>
							Con el propósito de dar cumplimiento al Artículo 12, de la RA 001/2013 Manual de
							Procedimientos{"\n"}
							Operativos de la UIF (Obligación de Conocer al Cliente Interno), proporciono mi Estado
							Patrimonial.
						</Text>
					</View>
				</View>
				...
				<View style={{ marginBottom: 10 }}>
					...
					<View style={S.row}>
						...
						<Text style={S.value}>
							{[empleado.av_calle_pasaje, empleado.zona_barrio].filter(Boolean).join(" — ")}
						</Text>
					</View>
					<View style={{ flexDirection: "row" }}>
						<View style={{ flex: 1 }}>
				...
				<Text style={S.sectionTitle}>
					6. Otros Bienes (Maquinaria, Computadora, Muebles, Electrodomésticos, etc.)
				</Text>
				<View style={{ border: "0.5 solid #ccc", marginBottom: 8 }}>
					<View style={S.tableHeader}>
						<Text style={[S.tableCell, { flex: 3, fontFamily: "Helvetica-Bold" }]}>
						...
						deudas.map((i) => (
							<View key={i.id} style={S.tableRow}>
								...
								<Text style={S.tableCell}>
									{i.fecha_vencimiento
										? new Date(i.fecha_vencimiento).toLocaleDateString("es-BO")
										: ""}
								</Text>
								<Text style={S.amount}>{fmt(Number(i.valor ?? 0))}</Text>
							</View>
				...
				<View style={{ border: "0.5 solid #003366", padding: 6, backgroundColor: "#f0f4f8", marginBottom: 8 }}>
					<Text style={{ fontSize: 7, textAlign: "center", fontFamily: "Helvetica-Bold" }}>
						Declaro que los datos son verdaderos y autorizo para que sean verificados cuando así lo requiera
						la Entidad
					</Text>
				</View>
