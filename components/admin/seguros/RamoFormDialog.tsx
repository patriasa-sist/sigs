...
import { crearRamo, actualizarRamo, obtenerRamosPadre } from "@/app/admin/seguros/ramos/actions";
...
export function RamoFormDialog({ ramo, onSuccess, triggerVariant = "default", iconOnly = false }: RamoFormDialogProps) {
		...
		if (open) {
			...
			obtenerRamosPadre()
				.then((padres) => {
					...
					setRamosPadre(ramo ? padres.filter((p) => p.id !== ramo.id) : padres);
				})
	...
	const onSubmit = async (values: FormValues) => {
		...
		try {
			...
			const result = isEditing ? await actualizarRamo(ramo.id, formData) : await crearRamo(formData);
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="sm:max-w-[500px]">
				<DialogHeader>
					<DialogTitle>{isEditing ? "Editar Ramo de Seguro" : "Nuevo Ramo de Seguro"}</DialogTitle>
					<DialogDescription>
						{isEditing
								...
								render={({ field }) => (
									<FormItem className="flex flex-row items-end space-x-3 space-y-0 pb-2">
										...
										<div className="space-y-1 leading-none">
											...
											<FormDescription>Contiene otros ramos</FormDescription>
										</div>
									</FormItem>
								)}
							/>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Input placeholder="Ej: Automotor" {...field} disabled={isSubmitting} />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										>
											<FormControl>
												<SelectTrigger>
													<SelectValue
														placeholder={
															loadingPadres ? "Cargando..." : "Seleccionar ramo padre"
														}
													/>
												</SelectTrigger>
											</FormControl>
										...
										<FormDescription>Categoría a la que pertenece este ramo</FormDescription>
										<FormMessage />
									</FormItem>
								)}
							/>
