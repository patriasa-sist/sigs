...
import { crearAseguradora, actualizarAseguradora } from "@/app/admin/seguros/aseguradoras/actions";
...
export function AseguradoraFormDialog({
	...
	const onSubmit = async (values: FormValues) => {
		...
		try {
			...
			const codigoNum = values.codigo?.trim() ? parseInt(values.codigo, 10) : null;
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="sm:max-w-[425px]">
				<DialogHeader>
					<DialogTitle>{isEditing ? "Editar Aseguradora" : "Nueva Aseguradora"}</DialogTitle>
					<DialogDescription>
						{isEditing
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Input type="number" placeholder="Ej: 101" {...field} disabled={isSubmitting} />
									</FormControl>
									<FormDescription>Código único de identificación (opcional)</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
