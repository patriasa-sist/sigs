...
import { desactivarRamo, reactivarRamo } from "@/app/admin/seguros/ramos/actions";
...
export function RamosTable({ data }: RamosTableProps) {
	...
	const organizedData = useMemo(() => {
		...
		const orphans = hijos.filter((h) => !h.ramo_padre_id || !padres.find((p) => p.id === h.ramo_padre_id));
	...
	const filteredData = useMemo(() => {
		return organizedData.filter((ramo) => {
			...
			if (search) {
				...
				return (
					ramo.nombre.toLowerCase().includes(searchLower) || ramo.codigo.toLowerCase().includes(searchLower)
				);
			}
					...
					<Table>
						...
						<TableBody>
							{filteredData.map((ramo) => (
								<TableRow key={ramo.id} className={ramo.isChild ? "bg-muted/30" : ""}>
									<TableCell>
										<code className="text-xs bg-muted px-2 py-1 rounded">{ramo.codigo}</code>
									</TableCell>
									<TableCell>
									...
									<TableCell>
										<div className="flex items-center justify-end gap-1">
											...
											<SoftDeleteDialog
												...
												onAction={() =>
													ramo.activo ? desactivarRamo(ramo.id) : reactivarRamo(ramo.id)
												}
												onSuccess={handleRefresh}
												warningText={
													ramo.es_ramo_padre
